...
$request->setNick("sandbox_b_26");
$tc = new TopClient();
$result = json_decode(json_encode($tc->execute($request)), true);

try{
    if(isset($result['code'], $result['subcode'])){
        if($result['code']==40){
            echo "缺少必要的传入参数\n";
            var_export($request);
            echo "\n";
        ...
        switch($result['subcode']){
            case 'isp.top-remote-connection-error':
                echo "未知错误\n";
                break;
            ...
            case 'isv.shop-service-error:SHOP_IS_NOT_EXIST':
                echo "传入的nick存在，但此用户没有店铺\n";
                break;
            case 'isv.invalid-parameter:user-without-shop':
                echo "传入的nick存在，但此用户没有店铺\n";
                break;
            default:
                var_export($result);
